...
class LSTMFollower(object):
    def __init__(self,n_agents, state_dim, action_dim, seq_len, action_lower_bound, e_dim,
        ...
        super(LSTMFollower, self).__init__()
        ...
        self.actors = [LSTMActor(self.state_dim, e_dim,self.action_dim, self.actor_output_activation).to(self.device)] * self.n_agents
    ...
    def train(self):
        ...
        for agent_id in range(self.n_agents):
            ...
            for b in range(self.batch_size):
                newactor_actions = []
                for agent_id in range(self.n_agents):
                    newactor_action_var = self.actors[agent_id](b_states_var[b][:, agent_id, :])
                    if self.use_cuda:
...
class LSTMActor(nn.Module):
    ...
    def __init__(self, state_dim, e_dim, output_size, output_activation, hidden_size=64, init_w=3e-3):
        ...
        # LSTM layer
        self.lstm1 = nn.LSTM(input_size=64, hidden_size=hidden_size, batch_first=True)
        # self.lstm2 = nn.LSTM(input_size=64, hidden_size=hidden_size, batch_first=True)
    ...
    def forward(self, state):
        ...
        out = self.fc1(state)
        # x = out[:,:-1,:]
        output1, (hn, cn) = self.lstm1(out)

        x1 = F.leaky_relu(out)
        out1 = nn.functional.leaky_relu(output1)
        # out2 = nn.functional.leaky_relu(output2)
        # out = out1 * output1 + output2 * out2 + x1 * out
        out = F.leaky_relu(x1 * output1 + out*out1)
