...
class LeaderPolicy(object):
    def __init__(self, s_dim, r_dim, b_dim, o_dim, e_dim, u_dim, mobi_dim, r_bound, b_bound, n_candidate_edge, memory_capacity, pred_len=2,
                 n_hidden=512, tau=0.001, lr_a=1e-4, lr_c=1e-4, batch_size=64, graph_mode='e2e'): #graph mode:e2e, u2e
        self.seq_len = int(mobi_dim)
        self.memory =  PrioritizedReplayBuffer(max_size=memory_capacity, state_dim=s_dim, action_dim=e_dim, n_agent=u_dim, seq_len=mobi_dim, mode='leader')  # a_dim=r_dim+o_dim
        self.n_agent = u_dim
        ...
        self.actor = Actor(s_dim, self.r_dim, self.e_dim, self.n_agent, n_hidden=n_hidden) # a_dim=n_candidate_edge*u_dim
    ...
    def load_model(self, path='ac_model.pth', target_critic=None):
        ...
        self.actor_critic = ActorCritic(self.s_dim, self.r_dim, self.e_dim, self.n_agent, n_hidden=self.n_hidden).to(self.device)
    ...
    def learn_td3(self, batch_size=6, gamma=0.99, gae_lambda = 0.95, algo_name='td3'):
        ...
        batch_id, batch_s, batch_a, batch_r, batch_s_, batch_log, batch_val, bter = self.memory.sample_buffer(batch_size)
...
class Actor(nn.Module):
    def __init__(self, s_dim, r_dim, e_dim, n_agent,mobi_dim=2, device=torch.device("cuda:0"),n_hidden=512,num_heads=1, graph_mode='e2e'):
        ...
        self.g_atten = GAttention(n_agent, r_dim, n_hidden)
        self.W_res = nn.Parameter(torch.rand(e_dim,e_dim)*((4/e_dim)**0.5)-(1/e_dim)**0.5)
        self.W_loc = nn.Parameter(torch.rand(e_dim,e_dim)*((4/e_dim)**0.5)-(1/e_dim)**0.5)
    ...
    def forward(self, r_in, offload_in):
        ...
        # offload_r = offload * r
        adj_res = self.W_res @ self.adj
        adj_loc = self.W_loc @ self.adj
        r = nn.functional.sigmoid(self.g_atten(r_in, adj_res))
        offload_r = self.g_atten(offload_in, adj_loc)
        r_atten = offload_r + r
        offload_out = self.offload_graph_transformer(r_atten, self.adj)
...
class Critic(nn.Module):
    ...
    def forward(self, s, a):
        ...
        if self.use_g_atte:
            a_clone = self.g_atten(a_clone, self.adj)
...
class ActorCritic(nn.Module):
    def __init__(self, s_dim, r_dim, e_dim, n_agent, mobi_dim=2, device=torch.device("cuda:0"),n_hidden=200,num_heads=1):
