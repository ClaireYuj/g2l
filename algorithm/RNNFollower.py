...
class RNNFollower(object):
    def __init__(self,n_agents, state_dim, action_dim, seq_len, action_lower_bound,e_dim,
                 ...
                 actor_output_activation=torch.relu, actor_lr=0.0001, critic_lr=0.001,
        ...
        super(RNNFollower, self).__init__()
        ...
        self.actors = [RNNActor(self.state_dim, self.e_dim, self.action_dim, self.actor_output_activation).to(self.device)] * self.n_agents
...
class RNNActor(nn.Module):
    ...
    def __init__(self, state_dim, e_dim, output_size, output_activation, hidden_size=64, init_w=3e-3,device=torch.device("cuda:0")):
        super(RNNActor, self).__init__()
        self.fc1 = nn.Linear(state_dim, 32)
        self.rnn1 = RNNBlock(input_size=state_dim, hidden_size=hidden_size, output_size=32)
    ...
    def forward(self, state):
        ...
        batch, seq_len, input_size = state.size()

        hidden = torch.zeros((self.rnn_hidden_size)).to(self.device)

        for t in range(seq_len):  # 时间步
            input_t = state[:, t, :]
            out, hidden = self.rnn1(input_t, hidden)
        # out = nn.functional.relu(self.rnn1(state))
        # out = torch.zeros(seq_len, 32).to(self.device)
        # out[0, :] = output
