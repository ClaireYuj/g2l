...
class IGNN(object):
    def __init__(self, s_dim, r_dim, b_dim, o_dim, e_dim, u_dim, mobi_dim, r_bound, b_bound, n_candidate_edge,
                 ...
                 n_hidden=128, tau=0.001, lr_a=1e-4, lr_c=1e-4, batch_size=64, graph_mode='e2e'):  # graph mode:e2e, u2e
        self.seq_len = int(mobi_dim)
        self.memory = PrioritizedReplayBuffer(max_size=memory_capacity, state_dim=s_dim, action_dim=e_dim,
                                              n_agent=u_dim, seq_len=mobi_dim, pred_len=pred_len,mode='leader')  # a_dim=r_dim+o_dim
    ...
    def train(self, batch_size=1, gamma=0.99, gae_lambda=0.95, algo_name='ppo'):
        ...
        for i in range(batch_size):
            ...
            for step in reversed(range(q_values.shape[1])):
                ...
                advantage[:, step] = gae
            ratio = torch.exp(next_actions_log - ba_log)[:,-self.pred_len:,:] + 1e-8  # 比率 r_t(θ)
...
class Actor(nn.Module):
    def __init__(self, s_dim, r_dim, e_dim, n_agent,mobi_dim=2, device=torch.device("cuda:0"),n_hidden=512,num_heads=1, graph_mode='e2e'):
        ...
        self.device = device
        self.offload_graph_transformer = GraphTransformer(feature_dim_size=(e_dim), num_classes=e_dim, hidden_size=n_hidden,nhead=1,
                                                            num_self_att_layers=1, num_GNN_layers=1).to(device) # input:
    ...
    def forward(self, r_in, offload_in):
        ...
        offload_r = offload_in + F.tanh(r_in)
        offload_r = F.tanh(offload_r)
        offload_out = self.offload_graph_transformer(offload_r, self.adj)
...
class ActorCritic(nn.Module):
    def __init__(self, s_dim, r_dim, e_dim, n_agent, mobi_dim=2, device=torch.device("cuda:0"), n_hidden=200,
        ...
        self.r_dim = r_dim
        self.s_dim = s_dim
        self.actor = Actor(s_dim=s_dim, r_dim=r_dim, e_dim=e_dim, n_agent=n_agent,n_hidden=n_hidden)
    ...
    def forward(self, s):
        ...
        action_out, action = self.actor(r, offload)
