...
class MAMAMBA(object):
    def __init__(self,n_agents, state_dim, action_dim, seq_len, action_lower_bound,e_dim,
        ...
        self.actors = [MambaActor(self.state_dim, e_dim, self.action_dim, self.actor_output_activation).to(self.device)] * self.n_agents

        critic_state_dim =  self.state_dim
        critic_action_dim = self.action_dim
        self.critics = [MambaCritic(critic_state_dim, critic_action_dim).to(self.device)] * 1
    ...
    def train(self):
        ...
        for agent_id in range(self.n_agents):
            next_action_var, _ = self.actors_target[agent_id](b_next_states_var[:, :, agent_id, :])
            if self.use_cuda:
        ...
        for agent_id in range(self.n_agents):
            ...
            newactor_actions = []
            for agent_id in range(self.n_agents):
                newactor_action_var, _ = self.actors[agent_id](b_states_var[:, :, agent_id, :])
                if self.use_cuda:
...
class MambaActor(nn.Module):
    ...
    def __init__(self, state_dim, e_dim, output_size, output_activation, hidden_size=64, init_w=3e-3):
        ...
        self.fc3.weight.data.uniform_(-init_w, init_w)
        self.fc3.bias.data.uniform_(-init_w, init_w)
        self.diff_f1 =  nn.Parameter(torch.tensor(0.5))
        self.diff_f2 =  nn.Parameter(torch.tensor(-0.5))
        # Activation function for the output
        self.output_activation = output_activation
    ...
    def forward(self, state):
        ...
        # out = torch.cat([delta_x1, delta_x2], dim=1) + x
...
class MambaCritic(nn.Module):
    ...
    def __call__(self, state, action):
        ...
        diff = nn.functional.relu(torch.cat((out[:,0:1,:], x), dim=1))
        ...
        out =  nn.functional.relu(self.fc3(diff))
        out = torch.transpose(out, 0, 1)
        out = out.flatten(1, 2)
...
class CriticNetwork(nn.Module):
    ...
    def __call__(self, state, action):
        ...
        if out.dim() == 2:
            out = out.unsqueeze(0)
        out = nn.functional.relu(self.mamba1(out))
